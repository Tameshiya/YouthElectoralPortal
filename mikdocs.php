...
<body>
  ...
  <div class="row category-header">
    <div class="col-sm-8">
            <h1 class="category-head">Документы</h1>
    </div>
    <div class="col-sm-4">
      <p class="breadcrumbs"><span class="white-bg-spn"><a href="index.php">Главная</a>  /  Документы</span></p>
    </div>
  </div>
  <?php
  $jsonurl = "http://127.0.0.1:8081/docs?scope=".$_GET['scope_id'];
  if (isset($_GET['version_id'])) {
    $jsonurl = $jsonurl.'&version='.$_GET['version_id'];
  }
  $json = file_get_contents($jsonurl);
  $result = json_decode($json, true);
  //печатаем ссылки на версии документов (года)
  echo '<ul id="versions">';
  for ($i = 0; isset($result['versions'][$i]); $i++) {
    if ($result['versions'][$i]['id'] == $result['version']) {
      echo '<li id="current">'.$result['versions'][$i]['name'].'<li>';
    } else {
      echo '<li><a href="?scope_id='.$_GET['scope_id'].'&version_id='.$result['versions'][$i]['id'].'">'.$result['versions'][$i]['name'].'</a></li>';
    }
  }
  echo '</ul><div style="margin-top:50px;"></div>';

  //Печатаем ссылки на документы
  for ($i = 0; isset($result['documents'][$i]); $i++) {
    echo '<a href="'.$result['documents'][$i]['url'].'"><p style="text-align: justify;">'.$result['documents'][$i]['name'].'</p></a>';
  }
  ?>
  </section>
  <footer>
